...
//根据证件号查
	...
	@Select("SELECT exam_name FROM exam WHERE type =#{type}")
	List<String> checksubjectAndlevel(@Param("type") String type);
	
	@Select("SELECT exam_name FROM exam WHERE type =#{type} and level =#{level}")
	List<String> checksubjectAndlevel1(@Param("type") String type,@Param("level") String level);
	...
	//根据名字+科目查询  gyd
	@Select("SELECT start_time,duration,question_num,total_grade FROM exam WHERE exam_name =#{name} group by exam_name")
	Map<String,Object> queryExamByNameSubject(@Param("name")String name);
	
	@Select("SELECT * FROM exam WHERE exam_name =#{name} and type=#{subject}")
	List<Exam> queryExamByNameSubject1(@Param("name")String name,@Param("subject")String subject);
...
//根据id查
	...
	@Select("SELECT * FROM hand_exam_store WHERE subject =#{subject}")
	List<HandExamStore> getHandListNoLevel(@Param("subject")String subject,@Param("level")String level);
	...
	@Select("SELECT * FROM hand_exam_store WHERE subject =#{subject} and type=#{type}")
	List<HandExamStore> getHandList1NoLevel(@Param("subject")String subject,@Param("type")int type,@Param("level")String level);
	...
	@Select("SELECT * FROM choose_select_store WHERE subject =#{subject}")
	List<ChooseSelectStore> getSelectNoLevel(@Param("subject")String subject,@Param("level")String level);
	...
	@Select("SELECT * FROM choose_select_store WHERE subject =#{subject} and type=#{type}")
	List<ChooseSelectStore> getSelect1NoLevel(@Param("subject")String subject,@Param("type")int type,@Param("level")String level);
	...
	@Select("SELECT * FROM decide_store WHERE subject =#{subject}")
	List<DecideStore> getDecideNoLevel(@Param("subject")String subject,@Param("level")String level);
	...
	@Select("SELECT * FROM decide_store WHERE subject =#{subject} and type=#{type}")
	List<DecideStore> getDecide1NoLevel(@Param("subject")String subject,@Param("type")int type,@Param("level")String level);
	...
	@Select("SELECT id,question_name,question_code,type,grade ,pass_table_id,picture_path,'comm' AS typeQ FROM comm_exam_store   UNION ALL "
			+ "SELECT id,question_name,question_code,type,grade ,pass_table_id,picture_path,'fill' AS typeQ FROM  fill_blank_exam_store where subject =#{subject}")
	List<NewExam> getHFNoLevel(@Param("subject")String subject,@Param("level")String level);
	...
	@Select("SELECT id,question_name,question_code,type,grade ,pass_table_id,picture_path,'fill' AS typeQ FROM  fill_blank_exam_store where subject =#{subject}")
	List<NewExam> getFillNoLevel(@Param("subject")String subject,@Param("level")String level);
	...
	@Select("SELECT id,question_name,question_code,type,grade ,pass_table_id,picture_path,'comm' AS typeQ FROM comm_exam_store where type=#{type}  UNION ALL "
			+ "SELECT id,question_name,question_code,type,grade ,pass_table_id,picture_path,'fill' AS typeQ FROM  fill_blank_exam_store "
			+ "where subject =#{subject} and type=#{type}")
	List<NewExam> getHF1NoLevel(@Param("subject")String subject,@Param("type")int type,@Param("level")String level);
	...
	@Insert({"INSERT INTO exam  (exam_id ,exam_name,id_number,type,status,grade,duration,total_grade,start_time,hand_exam_question,"
			...
			+ "choose_exam_question,choose_exam_result,choose_exam_answer,decide_exam_question,decide_exam_result,decide_exam_answer,create_exam,fill_num,single_choose_num,many_choose_num,decide_num,question_grade,level)"
			...
			+ "#{chooseExamQuestion},#{chooseExamResult},#{chooseExamAnswer},#{decideExamQuestion},#{decideExamResult},#{decideExamAnswer},#{createExam},#{fillNum},#{singleChooseNum},#{manyChooseNum},#{decideNum},#{questionGrade},#{level})"})
	int add(Exam exam);
...
//所有考试数	
	@Select("select count(*) from exam where exam_name =#{name}")
	int getExamNumByName(@Param("name")String name);
	
	//已考的次数
	@Select("select count(*) from exam where status = 1 and exam_name =#{name}")
	int getExamByStatusAndName(@Param("name")String name);
...
//差	
	...
	@Insert("<script>" +
			"insert into exam (exam_id,exam_name,id_number,type,status,duration,total_grade,start_time," +
			"fill_blank_exam_question,fill_blank_exam_result,question_num," +
			"choose_exam_question,choose_exam_result,decide_exam_question,decide_exam_result,question_grade,create_exam,fill_num,single_choose_num,many_choose_num,decide_num,level," +
			"hand_num,comm_num,comm_exam_question,comm_exam_result,hand_exam_question,hand_exam_result)" +
			"values" +
			"<foreach collection='list'  item='value'  separator=','>" +
			"(#{value.examId}, #{value.examName},#{value.idNumber},#{value.type},#{value.status},#{value.duration},#{value.totalGrade},#{value.startTime},#{value.fillBlankExamQuestion}," +
			"#{value.fillBlankExamResult},#{value.questionNum},#{value.chooseExamQuestion},#{value.chooseExamResult}," +
			"#{value.decideExamQuestion},#{value.decideExamResult},#{value.questionGrade},#{value.createExam},#{value.fillNum},#{value.singleChooseNum},#{value.manyChooseNum},#{value.decideNum},#{value.level}," +
			"#{value.handNum},#{value.commNum},#{value.commExamQuestion},#{value.commExamResult},#{value.handExamQuestion},#{value.handExamResult})"
			+ "</foreach>" +
            "</script>")
    ...
    //批量插入list集合的数据
	@Insert("<script>" +
			"insert into exam (exam_id ,exam_name,id_number,type,status,grade,duration,total_grade,start_time,hand_exam_question," +
			"fill_blank_exam_question,fill_blank_exam_result,comm_exam_question,comm_exam_result,question_num," +
			...
			"(#{item.examId},#{item.idNumber},#{item.grade},#{item.totalGrade},#{item.handExamQuestion},#{item.chooseExamQuestion},"
			+ "#{item.fillBlankExamQuestion},#{item.decideExamQuestion},#{item.commExamQuestion},#{item.handExamResult},#{item.chooseExamResult},"
			+ "#{item.decideExamResult},#{item.fillBlankExamResult},#{item.commExamResult},#{item.handExamAnswer},#{item.fillBlankExamAnswer},"
			+ "#{item.chooseExamAnswer},#{item.decideExamAnswer},#{item.commExamAnswer},#{item.examName},#{item.type},"
			+ "#{item.status},#{item.duration},#{item.questionNum},#{item.startTime},#{item.endTime},#{item.examPhoto})" +
			"</foreach>" +
			"</script>")
	@Update("<script>" +
			"update exam (exam_id ,exam_name,id_number,type,status,grade,duration,total_grade,start_time,hand_exam_question," +
			"fill_blank_exam_question,fill_blank_exam_result,comm_exam_question,comm_exam_result,question_num," +
			"choose_exam_question,choose_exam_result,choose_exam_answer,decide_exam_question,decide_exam_result,decide_exam_answer)" +
			"values" +
			"<foreach collection='list' item='item' index='index' separator=','>" +
			...
			"</foreach>" +
			"</script>")
	int insertList(Exam exam);

	/**
	 * 学员获取试题时间，就是考试开始时间
	 * @param exam
	 */
	@Update("update exam set start_time=#{startTime} where exam_id=#{examId}")
	void updateStartTime(Exam exam);
}
