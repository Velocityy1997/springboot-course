...
//查数量
	...
	@Insert({
			"INSERT INTO user(id,name,sex,id_number,type,password,class_id,level_id,photo,state) VALUES(#{id},#{name},#{sex},#{idNumber},"
					+ "#{type},#{password},#{classId},#{levelId},#{photo},#{state})" })
	int add(User user);
	...
	@Update("UPDATE user SET id = #{id},name = #{name},sex = #{sex},id_number =#{idNumber},birthday = #{birthday}, type = #{type},password = #{password},"
			...
			+ "errdecide_blank_collect_list = #{errdecideBlankCollectList},state=#{state} WHERE id = #{id}")
	...
	/*@Update("UPDATE user SET id = #{id},name = #{name},sex = #{sex},id_number =#{idNumber},birthday = #{birthday}, type = #{type},password = #{password},"
			...
			+ ",errhand_collect_list = #{errhandCollectList},errfill_blank_collect_list = #{errfillBlankCollectList},errcomm_collect_list = #{errcommCollectList}"
			+ ",choose_blank_collect_list = #{chooseBlankCollectList},decide_blank_collect_list = #{decideBlankCollectList},errchoose_blank_collect_list = #{errchooseCollectList},"
			+ "errdecide_blank_collect_list = #{errdecideBlankCollectList},state=#{state}, exam_names=#{examNames},finished_exam = #{finishedExam} WHERE id = #{id}")
	int updateById(User user);*/
	...
	/*更新用户的待考考试*/
	@Update("UPDATE  user  SET exam_names =#{examNames}  WHERE id_number=#{idNumber} ")
	int updateUserExam(@Param("examNames")String examNames, @Param("idNumber")String idNumber);

	/*更新用户的待考考试*/
	/*@Update("UPDATE  user  SET finished_exam =#{finishedExam}  WHERE id_number=#{idNumber} ")
	int updateUserFinishedExam(@Param("finishedExam")String finishedExam, @Param("idNumber")String idNumber);*/
	 ...
	 * @return
	 */
	@Select("<script>select exam_names,id_number from user where id_number in " +
			"<foreach collection='array'  item='value'  separator=',' open='(' close=')'>" +
			"#{value}" +
			"</foreach></script>")
	List<User> selectExamNames(String[] idNumber);
