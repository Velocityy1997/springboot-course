...
//							String time = list.get(i).getStartTime().toString();
						...
						if (list.get(i).getEndTime() != null) {
							time = list.get(i).getEndTime().toString();
						}
						...
						if (list.get(i).getTotalGrade() == null) {
							t1.setJudge("不合格");
						} else {
		...
		//				List<Exam> exams = examServiceImpl.getExamByName(name);
						Map<String,Object> exams = examServiceImpl.getExamByNameAndSub1(name);
                        int exNum = examServiceImpl.getExamNumByName(name);//所有考试
                        int yiNum = examServiceImpl.getExamByStatusAndName(name);//已考试
						...
						if(exams!= null) {
							model.setPeopleNum(yiNum + "/" + exNum);
							model.setGrade(String.valueOf(exams.get("total_grade")));
							model.setNums((Integer)exams.get("question_num"));
							Timestamp st = (Timestamp)exams.get("start_time");
							...
							newTime.add(Calendar.MINUTE,(Integer)exams.get("duration"));
...
//				}
		...
		public Map<String, Object> checkPapers(String examId) {
			...
			try {
				...
				map1.put("testData",modelList);
				map1.put("examPhoto","./识图用图/middle-map.png");
	...
	* @return
	 * by:gyd
	*/
	@ApiOperation(value="批量更新(测试)",notes="批量更新(测试)")
	@RequestMapping(value = "/batch",method = RequestMethod.POST)
	@ResponseBody
	 public Result batchHandle(String idNumber,String subject) {
		 ...
		 try {
			 ...
			 int tag = 0;
			 List<Exam>list = new  ArrayList<Exam>();
			 list = examServiceImpl.getExams(idNumber, subject);//测试数据
			 tag = examServiceImpl.batchUpateExam(list);
