...
public class ExamServiceImpl implements IExamService{
	...
	public Map<String,Object> getExamByNameAndSub1(String name) {
		...
		Map<String,Object> map = examMapper.queryExamByNameSubject(name);
	...
	//根据姓名&科目查询考试
		...
		public List<Exam> getExamByNameAndSub(String name,String subject) {
			...
			List<Exam> map = examMapper.queryExamByNameSubject1(name,subject);
	...
	public int getExamNumByName(String name) {
		...
		int nameList = examMapper.getExamNumByName(name);
	...
	public int getExamByStatusAndName(String name) {
		...
		int nameList = examMapper.getExamByStatusAndName(name);
	...
	public List<HandExamStore> getHandList(String subject,String level) {
		...
		if(level.equals("0")) {
			list = examMapper.getHandListNoLevel(subject,level);
		}
		else {
	...
	public List<HandExamStore> getHandList1(String subject,int type,String level) {
		...
		if(level.equals("0")) {
			list = examMapper.getHandList1NoLevel(subject,type, level);
		}
		else {
	...
	public List<TreeClass> getTreeList(String level) {
		...
		if(level == null) {
			...
			if ("0".equals(level)) {
				...
				treeList = examMapper.getAllTreeList( );
	...
	public List<NewExam> getHF(String subject,String level) {
		...
		if ("0".equals(level)) {
			list = examMapper.getHFNoLevel(subject, level);
		}
		else {
	...
	public List<NewExam> getFill(String subject,String level) {
		...
		if ("0".equals(level)) {
			list = examMapper.getFillNoLevel(subject, level);
		}
		else {
	...
	public List<NewExam> getHF1(String subject,int cate,String level) {
		...
		if ("0".equals(level)) {
			list = examMapper.getHF1NoLevel(subject,cate, level);
		}
		else {
	...
	public List<ChooseSelectStore> getSelect(String subject,String level) {
		...
		if ("0".equals(level)) {
			list = examMapper.getSelectNoLevel(subject, level);
		}
		else {
	...
	public List<ChooseSelectStore> getSelect1(String subject,int type,String level) {
		...
		List<ChooseSelectStore> list = new ArrayList<ChooseSelectStore>();
		if ("0".equals(level)) {
			list = examMapper.getSelect1NoLevel(subject, type, level);
	...
	public List<DecideStore> getDecide(String subject,String level) {
		...
		if ("0".equals(level)) {
			list = examMapper.getDecideNoLevel(subject, level);
	...
	public List<DecideStore> getDecide1(String subject,int type,String level) {
		...
		List<DecideStore> list = new ArrayList<DecideStore>();
		if ("0".equals(level)) {
			list = examMapper.getDecide1NoLevel(subject,type, level);
		}
		else {
	...
	@Override
		...
		public  boolean checkExam(String subject,String level) {
			...
			boolean tag = false;
            if( !subject.equals("3")) {
            	list =  examMapper.checksubjectAndlevel(subject);

            }
            else {
            	list =  examMapper.checksubjectAndlevel1(subject,level);
	...
	@Override
	public int addExam(List<Exam> examList) {

		return examMapper.addExam(examList);
	}
