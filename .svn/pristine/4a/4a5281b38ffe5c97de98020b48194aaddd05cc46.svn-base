...
class Drag {
  ...
  initPos() {
    ...
    if (this.targetObj.id == 'circlrRuleBox') {
      this.width = this.targetObj.offsetWidth
      this.height = this.targetObj.offsetHeight
    } else {
      this.width = this.targetObj.offsetWidth * .4
      this.height = this.targetObj.offsetHeight * .4
    }
    ...
    switch (this.type) {
      case 0:
        if (this.rotateFlag) {
          ...
          // 更改
          if (target.id == 'circlrRuleBox') {
            target.style.transform = `rotate(${this.angle}deg) scale(1)`
            this.pannelDom.style.transformOrigin = 'bottom center'
          } else {
            target.style.transform = `rotate(${this.angle}deg) scale(.4)`
            this.pannelDom.style.transformOrigin = 'left top'
          }
          this.pannelDom.style.transform = `rotate(${this.angle}deg)`
